...
def keccak256(data: bytes) -> bytes:
    """
    Compute the Keccak-256 digest (Ethereum ``keccak256``, not NIST SHA-3).

    Args:
        data: Arbitrary-length input; may be empty.

    Returns:
        32-byte digest.

    Note:
        This is a pure-Python reference implementation; the Cython extension
        under :mod:`cycel.crypto.hashes.keccak` is preferred when installed.
    """
    rate_bytes = 1088 // 8
