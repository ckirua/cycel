...
cpdef bytes privkey_to_pubkey(bytes privkey):
    ...
    Args:
        privkey: 32-byte big-endian private key scalar.

    Returns:
        Uncompressed public key bytes.

    Raises:
        ValueError: If ``privkey`` length is not 32 or the scalar is invalid.
    """
    if len(privkey) != 32:
...
cpdef bytes recover_pubkey(bytes msg_hash, object r, object s, int recid):
    """
    Recover the uncompressed public key from a ECDSA signature over ``msg_hash``.

    Args:
        msg_hash: 32-byte message digest (big-endian field element semantics).
        r: Signature ``r`` component (integer).
        s: Signature ``s`` component (integer).
        recid: Recovery id in ``{0, 1, 2, 3}`` (compressed y / overflow semantics).

    Returns:
        65-byte uncompressed public key.

    Raises:
        ValueError: On invalid inputs or non-recoverable signature data.
    """
    if len(msg_hash) != 32:
...
cpdef tuple sign_recoverable(bytes privkey, bytes msg_hash):
    """
    Produce a recoverable ECDSA signature over ``msg_hash`` using ``privkey``.

    The recovery id is encoded in the third return value as ``v = 27 + recid``
    (Ethereum convention).

    Args:
        privkey: 32-byte private key.
        msg_hash: 32-byte digest to sign.

    Returns:
        Tuple ``(r, s, v)`` with ``r``, ``s`` as integers and ``v`` in
        ``{27, 28, 29, 30}``.

    Raises:
        ValueError: If lengths are wrong or a signature cannot be constructed.
    """
...
cpdef str privkey_to_address(bytes privkey):
    """
    Ethereum address ``0x`` + 40 hex chars (last 20 bytes of Keccak-256(pubkey)).

    Args:
        privkey: 32-byte secp256k1 private key.

    Returns:
        Lowercase hex string with ``"0x"`` prefix (40 hex digits).
    """
