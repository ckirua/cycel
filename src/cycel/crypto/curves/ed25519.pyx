...
cpdef bytes ed25519_public_key(bytes seed):
    ...
    Args:
        seed: 32-byte secret seed (same width as a private scalar in this API).

    Returns:
        Compressed public key encoding (32 bytes).
...
cpdef bytes ed25519_sign(bytes message, bytes seed):
    """
    Sign ``message`` with the Ed25519 key expanded from ``seed``.
    ...
    Returns:
        64-byte signature ``R || S`` (little-endian packed).
...
cpdef bint ed25519_verify(bytes message, bytes signature, bytes public_key):
    """
    Verify an Ed25519 signature produced by :func:`ed25519_sign`.
    ...
    Returns:
        ``True`` if the signature is valid; ``False`` on any parse or
        cryptographic failure (no exception on invalid signatures).
    """
