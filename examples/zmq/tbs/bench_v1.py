...
Transport: **PUSH / PULL** (not PUB/SUB)
---------------------------------------
PUB/SUB can **silently drop** messages when the subscriber falls behind. The
publisher still finishes its counted send loop, but the subscriber keeps
waiting for ``recv`` forever — often fine for a few hundred messages on
localhost, then “eternity” at a few thousand. The ZMQ guide calls out slow
joiners and lossy fan-out for PUB/SUB.

This bench uses **PULL bind + PUSH connect** so every sent multipart is
queued for the peer (until HWM): **send count == recv count**, matching
``asyncio.gather`` + fixed **message counts** (warmup + measured).

Throughput
    Pusher sends ``warmup_messages + measure_messages``; puller drops the
    warmup receives, then times ``measure_messages`` receives.

Latency
    Same counting model; puller records one-way ``perf_counter()`` stamp
...
from cycel.zmq.zmq_async import ZMQPull, ZMQPush
...
async def _run_throughput(
    *,
    ...
    """PULL binds first; PUSH connects after ``bind_ready`` (see ZMQ PUSH/PULL)."""
    params_pull = SimpleNamespace(url=url)
    params_push = SimpleNamespace(url=url)
    bind_ready = asyncio.Event()

    async def puller() -> tuple[int, int, float]:
        async with ZMQPull(params_pull) as pl:
            bind_ready.set()
            for _ in range(warmup_n):
                await pl.recv_multipart_async()
            total_bytes = 0
            t0 = time.perf_counter()
            for _ in range(measure_n):
                frames = await pl.recv_multipart_async()
                for f in frames:
                    total_bytes += len(f)
    ...
    async def pusher() -> None:
        await bind_ready.wait()
        await asyncio.sleep(0.02)
        async with ZMQPush(params_push) as ps:
            for _ in range(warmup_n + measure_n):
                await ps.send_multipart_async(topic, payload)

    (n_msg, total_bytes, dt), _ = await asyncio.gather(puller(), pusher())
    ...
    async def pusher() -> None:
        ...
        async with ZMQPush(params_push) as ps:
            for _ in range(warmup_n + sample_n):
                ...
                await ps.send_multipart_async(topic, body)

    lat_ms, _ = await asyncio.gather(puller(), pusher())
...
async def _cmd_throughput(args: argparse.Namespace) -> None:
    ...
    _emit_result(meta, merged, as_json=args.json)
